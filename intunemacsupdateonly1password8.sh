...
if [ "$AppID" = "1password8" ]; then
    echo "AppID is 1password8 — switching to intunemacsdev.sh"

    tmpDev="/var/tmp/intunemacsdev.sh"
    if ! /usr/bin/curl -fsSL "${REPO_URL}/intunemacsdev.sh" -o "$tmpDev"; then
        echo "ERROR: Failed to download intunemacsdev.sh"
        exit 1
    fi

    # Apply same placeholder substitutions the server normally performs
    # (only when values are non-empty so we don't clobber defaults)
    # Note: careful with sed delimiters/escapes for paths & quotes.
    esc() { printf '%s' "$1" | sed -e 's/[\/&]/\\&/g'; }

    [ -n "$run" ] && sed -i '' "s/run=\"\"/run=\"$(esc "$run")\"/g" "$tmpDev"
    [ -n "$item" ] && sed -i '' "s/item=\"\"/item=\"$(esc "$item")\"/g" "$tmpDev"
...
if [[ $(sw_vers -buildVersion | cut -c1-2) -lt 20 ]] || [ "$enabledialog" = "false" ]; then
    ...
    if [[ "$icon" == "" ]]; then
        appPathDetected=$(mdfind "kind:application AND name:$itemName" | head -1 || true)
        appIcon=$(defaults read "${appPathDetected}/Contents/Info.plist" CFBundleIconFile || true)
        if [[ "$(echo "$appIcon" | grep -io ".icns")" == "" ]]; then
            appIcon="${appIcon}.icns"
        fi
        icon="${appPathDetected}/Contents/Resources/${appIcon}"
        if [ ! -f "${icon}" ]; then
            case $LOGO in
                appstore)
