...
# >>> Added: per-label extra args (used for 1password8 workaround)
EXTRA_ARGS=""
...
# >>> Added: 1Password 8 workaround BEFORE we call Installomator for the item
if [[ "$item" == "1password8" ]]; then
    # If a pkg receipt exists and reports version 0 (or empty), forget it to avoid misleading detections
    ...
    # Force Installomator to use the app bundle version (blank packageID switches detection mode)
